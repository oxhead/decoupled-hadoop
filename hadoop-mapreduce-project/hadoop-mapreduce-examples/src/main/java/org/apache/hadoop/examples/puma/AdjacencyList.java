...
/**
 * This is an example Hadoop Map/Reduce application. It produces list of
 * out-links and in-links for each host. Map reads the text input files each
 * line of the format {A1,A2}, and produces <A1,from{}:to{A2}> and
 * <A2,from{A1}:to{}> tuples as Map output. The Reduce output is a union of all
 * such lists with same key.
 * 
 * 
 * To run: bin/hadoop jar build/hadoop-examples.jar adjlist [-m <i>maps</i>] [-r
 * <i>reduces</i>] <i>in-dir</i> <i>out-dir</i>
 * 
 * @author Faraz Ahmad
 */
...
public class AdjacencyList extends Configured implements Tool {
	...
	public static final Log LOG = LogFactory.getLog(AdjacencyList.class);
			...
			while (values.hasNext()) {
				...
				if (!fromLine.isEmpty()) {
					...
					while (itr.hasMoreTokens()) {
						...
						if (!fromList.contains(vertex) && fromList.size() < LIMIT) // avoid
																					// heap
																					// overflow
							fromList.add(vertex);
					}
				...
				if (!toLine.isEmpty()) {
					...
					while (itr.hasMoreTokens()) {
						vertex = new String(itr.nextToken());
						if (!toList.contains(vertex) && toList.size() < LIMIT) // avoid
																				// heap
																				// overflow
							toList.add(vertex);
					}
	...
	public static void main(String[] args) throws Exception {
		int ret = ToolRunner.run(new AdjacencyList(), args);
		System.exit(ret);
	}
}
