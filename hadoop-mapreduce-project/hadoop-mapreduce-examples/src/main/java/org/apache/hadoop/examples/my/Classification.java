...
public class Classification {
  ...
  public static int main(String[] args) throws Exception {
    ...
    for (i = 0; i < otherArgs.length; ++i) {
      try {
        if ("-m".equals(otherArgs[i])) {
          numMaps = Integer.parseInt(otherArgs[++i]);
        } else if ("-r".equals(otherArgs[i])) {
          numReds = Integer.parseInt(otherArgs[++i]);
        } else {
    ...
    Date startIteration;
    Date endIteration;
    JobConf jobConf = new JobConf(conf, Classification.class);
    jobConf.setJobName("classification");
    ...
    jobConf.setNumReduceTasks(numReds);
