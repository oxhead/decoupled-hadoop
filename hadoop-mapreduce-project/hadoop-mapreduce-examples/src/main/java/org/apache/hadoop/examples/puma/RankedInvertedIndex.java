...
/**
 * This is an example Hadoop Map/Reduce application. Map reads input data
 * containing each line of the format <word|file n> showing how many times (n)
 * each word appears in each file. Map separates count from the rest of the data
 * in the input. Map output format is <word,<file,n>>. Reduce emits <word,
 * list<n1, file1>, <n2, file2> ... > in decreasing order of occurrence of the
 * word in the respective files.
 * 
 * 
 * To run: bin/hadoop jar build/hadoop-examples.jar rankedinvertedindex [-m
...
public class RankedInvertedIndex extends Configured implements Tool {
	...
	public static class MapClass extends MapReduceBase implements Mapper<LongWritable, Text, Text, FileCountPair> {
		...
		public void map(LongWritable key, Text value, OutputCollector<Text, FileCountPair> output, Reporter reporter) throws IOException {
	...
	public static class Reduce extends MapReduceBase implements Reducer<Text, FileCountPair, Text, Text> {
		...
		public void reduce(Text key, Iterator<FileCountPair> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
