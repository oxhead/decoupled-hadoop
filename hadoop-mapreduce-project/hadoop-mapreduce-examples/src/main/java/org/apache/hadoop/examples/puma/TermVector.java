 ...
 * files, breaks each line into words Map output : <host, termVector> where host
 * is extracted from filename, termVector is a string <word:1> for all words in
 * the input file Reduce: counts all occurrences of all words in each file and
 * emits for each host, all those words which occur above a threshold specified
 * by CUTOFF. output format: <host, list<termVectors>>
 * 
 * To run: bin/hadoop jar build/hadoop-examples.jar termvectorperhost [-m
...
public class TermVector extends Configured implements Tool {

	private enum Counter {
		WORDS, VALUES
	}

	public static final Log LOG = LogFactory.getLog(TermVector.class);
	...
	public static class Combine extends MapReduceBase implements Reducer<Text, Text, Text, Text> {
		...
		public void reduce(Text key, Iterator<Text> values, OutputCollector<Text, Text> output, Reporter reporter) throws IOException {
			...
			LOG.fatal("Used heap space: " + (runtime.totalMemory() - runtime.freeMemory()) / mb);
	...
	/**
	 * A reducer class that adds term vectors, throws away infrequent terms
	 * (occuring less than CUTOFF times) and emits a final (host,termvector) =
	 * (host,(word,frequency)) pair.
	 */
		...
		static Map<String, IntWritable> sortByValue(Map<String, IntWritable> map) {
			...
			Collections.sort(list, new Comparator<Object>() {
				public int compare(Object o1, Object o2) {
					return -((IntWritable) ((Map.Entry<String, IntWritable>) (o1)).getValue()).compareTo((IntWritable) ((Map.Entry<String, IntWritable>) (o2)).getValue());
				}
			});
	...
	public int run(String[] args) throws Exception {
		...
		JobConf conf = new JobConf(TermVector.class);
		conf.setJobName("termvector");
	...
	public static void main(String[] args) throws Exception {
		int ret = ToolRunner.run(new TermVector(), args);
		System.exit(ret);
	}
}
